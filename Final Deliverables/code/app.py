...
import expenze_reports
import expenze_account
import sendmail
...
def index():
    ...
    if request.method == "GET":
        ...
        categories = expenze_categories.getSpendCategories(session["user_id"])

        # Get the users payers (for quick expense modal)
        payers = expenze_account.getPayers(session["user_id"])

        # Get todays date (for quick expense modal)
        date = datetime.today().strftime('%Y-%m-%d')

        # Get the users income
        income = expenze_account.getIncome(session["user_id"])

        # Get current years total expenses for the user
        expenses_year = expenze_dashboard.getTotalSpend_Year(session["user_id"])

        # Get current months total expenses for the user
        expenses_month = expenze_dashboard.getTotalSpend_Month(
            session["user_id"])
        
        # Get current week total expenses for the user
        expenses_week = expenze_dashboard.getTotalSpend_Week(session["user_id"])

        # Get last 5 expenses for the user
        expenses_last5 = expenze_dashboard.getLastFiveExpenses(
            session["user_id"])

        # Get every budgets spent/remaining for the user
        budgets = expenze_dashboard.getBudgets(session["user_id"])

        # Get weekly spending for the user
        weeks = expenze_dashboard.getLastFourWeekNames()
        spending_week = expenze_dashboard.getWeeklySpending(
            weeks, session["user_id"])

        # Get monthly spending for the user (for the current year)
        spending_month = expenze_dashboard.getMonthlySpending(
            session["user_id"])

        # Get spending trends for the user
        spending_trends = expenze_dashboard.getSpendingTrends(
            session["user_id"])

        # Get payer spending for the user
        payersChart = expenze_reports.generatePayersReport(session["user_id"])
        
    # email send
        if (expenses_month != None):
            if(expenses_month >= income):
                u_name = expenze_account.getUsername(session["user_id"])
                sendmail.send_mail(u_name,income)
    ...
    # User reached route via POST
        ...
        expenses = expenze_expenses.addExpenses(formData, session["user_id"])
...
def expensehistory():
    ...
    if request.method == "GET":
        ...
        history = expenze_expenses.getHistory(session["user_id"])
    ...
    # User reached route via POST
        ...
        oldExpense = expenze_expenses.getExpense(
        ...
        if userHasSelected_deleteExpense == True:
            ...
            deleted = expenze_expenses.deleteExpense(
            ...
            history = expenze_expenses.getHistory(session["user_id"])
            categories = expenze_categories.getSpendCategories(
                session["user_id"])
            payers = expenze_account.getPayers(session["user_id"])
        ...
        # Update the existing expense record
            ...
            expensed = expenze_expenses.updateExpense(
...
def budgets(year=None):
    ...
    if request.method == "GET":
        ...
        budgets = expenze_budgets.getBudgets(session["user_id"])

        # Get the users total budgeted amount
        budgeted = expenze_budgets.getTotalBudgetedByYear(
    ...
    # User reached route via POST
        ...
        # Delete the budget
        deletedBudgetName = expenze_budgets.deleteBudget(
...
def createbudget():
    ...
    if request.method == "POST":
        ...
        budgetDict = expenze_budgets.generateBudgetFromForm(formData)
        ...
        if "apology" in budgetDict:
            ...
            budget = expenze_budgets.createBudget(
        ...
        # Get the users total budgeted amount
        budgeted = expenze_budgets.getTotalBudgetedByYear(session["user_id"])
...
def updatebudget(urlvar_budgetname):
    ...
    if request.method == "POST":
        ...
        if "apology" in budgetDict:
            ...
            budget = expenze_budgets.updateBudget(
    ...
    # User reached route via GET
        ...
        budgetID = expenze_budgets.getBudgetID(
            urlvar_budgetname, session["user_id"])
        if budgetID is None:
            return apology("'" + urlvar_budgetname + "' budget does not exist")
        else:
            budget = expenze_budgets.getBudgetByID(budgetID, session["user_id"])
        ...
        budget = expenze_budgets.getUpdatableBudget(budget, session["user_id"])
...
def categories():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_newCategory:
            ...
            categoryCount = len(
                expenze_categories.getSpendCategories(session["user_id"]))
            if categoryCount >= 30:
            ...
            categoryID = expenze_categories.getCategoryID(newCategoryName)
            ...
            if categoryID:
                ...
                existingID = expenze_categories.getCategoryID(
                ...
                # Add the category to the users account
                    ...
                    expenze_categories.addCategory_User(
            ...
            # Category does not exist in the DB already - create a new category and then add it to the users account
                ...
                newCategoryID = expenze_categories.addCategory_DB(
                    newCategoryName)
        ...
        if userHasSelected_renameCategory:
            ...
            oldCategoryID = expenze_categories.getCategoryID(oldCategoryName)
            ...
            # Check to see if the *new* name already exists in the database (None == does not exist)
            newCategoryID = expenze_categories.getCategoryID(newCategoryName)
            ...
            if newCategoryID:
                ...
                newCategoryNameFromDB = expenze_categories.getSpendCategoryName(
                    newCategoryID)

                # Rename the category
                expenze_categories.renameCategory(
        ...
        if userHasSelected_deleteCategory:
            ...
            categoryID = expenze_categories.getCategoryID(deleteName)
            ...
            # Delete the category
            expenze_categories.deleteCategory(categoryID, session["user_id"])
    ...
    # User reached route via GET
        ...
        categoryBudgets = expenze_categories.getBudgetsSpendCategories(
            session["user_id"])

        # Generate a single data structure for storing all categories and their associated budgets
        categoriesWithBudgets = expenze_categories.generateSpendCategoriesWithBudgets(
...
def budgetsreport(year=None):
    ...
    budgets = expenze_reports.generateBudgetsReport(session["user_id"], year)
...
def monthlyreport(year=None):
    ...
    monthlySpending = expenze_reports.generateMonthlyReport(
        session["user_id"], year)
...
def spendingreport(year=None):
    ...
    spendingReport = expenze_reports.generateSpendingTrendsReport(
        session["user_id"], year)
...
def payersreport(year=None):
    ...
    payersReport = expenze_reports.generatePayersReport(
        session["user_id"], year)
...
def updateaccount():
    ...
    if request.method == "POST":
        ...
        if userHasSelected_updateIncome:
            ...
            updatedIncome = expenze_account.updateIncome(
        ...
        if userHasSelected_addPayer:
            ...
            # Add the payer
            newPayer = expenze_account.addPayer(newName, session["user_id"])
        ...
        if userHasSelected_renamePayer:
            ...
            # Rename the payer
            renamedPayer = expenze_account.renamePayer(
        ...
        if userHasSelected_deletePayer:
            ...
            # Delete the payer
            deletedPayer = expenze_account.deletePayer(name, session["user_id"])
        ...
        if userHasSelected_updatePassword:
            ...
            updatedPassword = expenze_account.updatePassword(request.form.get(
        ...
        user = expenze_account.getAllUserInfo(session["user_id"])
